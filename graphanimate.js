...
function plot(svgid,x,ys,joinpoints=false,xmanualinput=true,ymanualinput=false,xstart=0,xend=15,ystart=0,yend=10,xtitle="",ytitle="",height=1000,width=1000,pointcolours=["red"],plottype="circle",shade=false,xlabels=5,ylabels=5,xlinesperlabel=2,ylinesperlabel=2,yseriestitles=0,atitlesize=0.05,labelsize=0.04,pointsizes=[4]){
	...
	for (m=0;m<ys.length;m++){
		...
		for (k=0;k<xcoords.length;k++){
			...
			if (k>0 && joinpoints){
				...
				y2=ycoords[k]
				x2=xcoords[k]
				chart.innerHTML+="<line x1="+(x1) +" y1="+(y1) +" x2="+(x2) +" y2="+(y2) +" stroke=\""+pointcolour+"\" stroke-width=\"2\"/>"
			}
		}
...
xxx=[0,0.1]
yyy=[[3,3.1]]
function addxyelements(){
	xxx[xxx.length]=xxx[xxx.length-1]+0.2
	yyy[0][yyy[0].length]=Math.sin(xxx[xxx.length-1])+3
	if(yyy[0][yyy[0].length-1]>20){
		clearInterval(animate)
	}
	if(xxx[xxx.length-1]>15){
		clearInterval(animate)
	}
	plot("chart",xxx,yyy,true)
}
animate=setInterval(addxyelements,10)